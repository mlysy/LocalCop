...
#' @param all_out Logical; whether or not to output all fitted models or just the selected family/bandwidth combination.  See \strong{Value}.
#' @return If \code{all_out = TRUE}, a list with elements \code{family} and \code{bandwidth} containing the selected value of each.  Otherwise, a list with the following elements:
#' \describe{
...
CondiCopSelect <- function(u1, u2, family, X, xind = 100,
  ...
  fun <- function(ii) {
    CondiCopLikCV(u1=u1, u2=u2, family = gridVal$family[ii],
                  ...
                  optim_fun = optim_fun, cveta_out = all_out, cl = NA)
  }
