...
```{r localseq, message=FALSE, fig.align='center', fig.width=7, fig.height=5, fig.cap="Conditional Kendall $\\tau$ estimates under the Gumbel copula using bandwidth $h=0.1$."}
x0 <- seq(0, 1, by=0.02)
...
system.time({
  cvselect1 <- CondiCopSelect(u1= udata[,1], u2 = udata[,2],
...
system.time({
  cvselect2 <- CondiCopSelect(u1= udata[,1], u2 = udata[,2],
                              X = X, family = famset, band = bandset,
                              xind = n)
})
```
...
cv_res <- bind_rows(cv_res1, cv_res2) %>% 
  mutate(n = factor(c(rep(paste0("n=",xind), nrow(cv_res1)), 
         ...
         band = factor(band), 
         family = factor(family, levels = c(1,2,3,4,5), labels = c("Gaussian", "Student", "Clayton", "Gumbel", "Frank")))
...
x0 <- seq(0, 1, by=0.01)
copcol <- c(3,4,5,2,6)
fitseq <- sapply(1:5, function(ii) CondiCopLocFit(u1 = udata[,1], u2 = udata[,2], 
                         family = ii, X = X, x = x0, 
                         degree = 1, band = band))
plot(X, tau_true, type = "l", lwd=2, ylim=c(-0.5,1), ylab=expression(tau("x")))
for(ii in 1:5){
  tauest <- BiCopEta2Tau(fitseq[,ii]$eta, family=ii)
  lines(x0[tauest>0.01], tauest[tauest>0.01], col=copcol[ii])
}
```

The results suggest that misspecification of the copula family does not drastically affect the local likelihood estimates of the conditional Kendall's tau, except for the Clayton copula (in cyan) which shows the most departure from the data generating Gumbel copula.
