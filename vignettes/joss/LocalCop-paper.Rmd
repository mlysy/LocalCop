...
- `CondiCopSelect()`: For selecting a copula family and bandwidth parameter using leave-one-out cross-validation (LOO-CV) as described in @ACL2019.
...
In Figure \@ref(fig:copcomp), we compare the true conditional Kendall $\tau$ to estimates using each of the three conditional copula fitting packages **LocalCop**, **gamCopula**, and **CondCopulas**.  Note than **CondCopulas** does not provide a mechanism for family and bandwidth selection, so we used the ones provided by `LocalCop::CondiCopSelect()`.
